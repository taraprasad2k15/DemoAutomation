...
export class VFInsideIframeTestcaseSpec extends BaseSpec {
	public runTest() {
		...
		let VFframeId = ["(//iframe)[1]", "(//iframe)[1]"];

		this.zephyrTest('AutomationAPI', 'PROT-1274', 'FS_MAP_SF_DT_CX_PICKLIST_VF_INSIDE_IFRAME_INPUT.SET', () => {
			...
			this.step('Mapping Case Origin field', async () => {
				...
				const frame1 = await this.findElement(remoteWebdriver, "(//iframe)[1]");
				remoteWebdriver.switchTo().frame(frame1);
				console.log("switched to first frame")
				const frame2 = await this.findElement(remoteWebdriver, "(//iframe)[1]");
				remoteWebdriver.switchTo().frame(frame2); 
				console.log("switched to second frame")
				remoteWebdriver.switchTo().frame(0); 
				console.log("switched to 3 frame")
				const selectBtWE: WebElement = await this.findElement(remoteWebdriver, "(//input[@value='Select' and @type='button'])[last()]", 20 * 1000);
				selectBtWE.click();
		...
		this.zephyrTest('AutomationAPI', 'PROT-1275', 'FS_MAP_SF_DT_CX_PICKLIST_VF_INSIDE_IFRAME_INPUT.SETBYINDEX', () => {
			beforeEach(() => {
				this.beforeEach();
			});
			this.step('Mapping Case Origin field', async () => {
		...
		this.zephyrTest('AutomationAPI', 'PROT-1276', 'FS_MAP_SF_DT_CX_TEXT_VF_INSIDE_IFRAME_INPUT.SET', () => {
			beforeEach(() => {
				this.beforeEach();
			});
		...
		this.zephyrTest('AutomationAPI', 'PROT-1277', 'FS_MAP_SF_DT_CX_TEXTAREA_VF_INSIDE_IFRAME_INPUT.SET', () => {
			beforeEach(() => {
				this.beforeEach();
			});
		...
		this.zephyrTest('AutomationAPI', 'PROT-1278', 'FS_MAP_SF_DT_CX_CHECKBOX_VF_INSIDE_IFRAME_INPUT.CHECK', () => {
			beforeEach(() => {
				this.beforeEach();
			});
		...
		this.zephyrTest('AutomationAPI', 'PROT-1279', 'FS_MAP_SF_DT_CX_CHECKBOX_VF_INSIDE_IFRAME_INPUT.UNCHECK', () => {
			beforeEach(() => {
				this.beforeEach();
			});
			this.step('Mapping Checkbox field', async () => {
		...
		this.zephyrTest('AutomationAPI', 'PROT-1280', 'FS_MAP_SF_DT_CX_CHECKBOX_VF_INSIDE_IFRAME_INPUT.TOGGLE', () => {
			beforeEach(() => {
				this.beforeEach();
			});
			this.step('Mapping Checkbox field', async () => {
		...
		this.zephyrTest('AutomationAPI', 'PROT-1281', 'FS_MAP_SF_DT_CX_TIME_VF_INSIDE_IFRAME_INPUT.SET', () => {
			beforeEach(() => {
				this.beforeEach();
			});
		...
		this.zephyrTest('AutomationAPI', 'PROT-1282', 'FS_MAP_SF_DT_CX_MULTI SELECT PICKLIST_VF_INSIDE_IFRAME_INPUT.SELECT', () => {
			beforeEach(() => {
				this.beforeEach();
			});
		...
		this.zephyrTest('AutomationAPI', 'PROT-1283', 'FS_MAP_SF_DT_CX_MULTI SELECT PICKLIST_VF_INSIDE_IFRAME_INPUT.DESELECT', () => {
			beforeEach(() => {
				this.beforeEach();
			});
		...
		this.zephyrTest('AutomationAPI', 'PROT-1284', 'FS_MAP_SF_DT_CX_DATE_VF_INSIDE_IFRAME_INPUT.SET', () => {
			beforeEach(() => {
				this.beforeEach();
			});
			this.step('Mapping Custom Date field', async () => {
				...
				this.assertTestBuilderState(pageStructureInfo, VFTestcase.customDate.fieldDetails, Standard.interaction.set, Standard.applicableInteractions.Text.enterableMode, response.testbuilderState);
			});
		...
		this.zephyrTest('AutomationAPI', 'PROT-1285', 'FS_MAP_SF_DT_CX_DATETIME_VF_INSIDE_IFRAME_INPUT.SET', () => {
			beforeEach(() => {
				this.beforeEach();
			});
		...
		this.zephyrTest('AutomationAPI', 'PROT-1286', 'FS_MAP_SF_DT_CX_RADIOBT_VF_INSIDE_IFRAME_INPUT.CHECK', () => {
			beforeEach(() => {
				this.beforeEach();
			});
		...
		this.zephyrTest('AutomationAPI', 'PROT-1287', 'FS_MAP_SF_DT_CX_CURRENCY_VF_INSIDE_IFRAME_INPUT.SET', () => {
			beforeEach(() => {
				this.beforeEach();
			});
		...
		this.zephyrTest('AutomationAPI', 'PROT-1288', 'FS_MAP_SF_DT_CX_NUMBER_VF_INSIDE_IFRAME_INPUT.SET', () => {
			beforeEach(() => {
				this.beforeEach();
			});
		...
		this.zephyrTest('AutomationAPI', 'PROT-1289', 'FS_MAP_SF_DT_CX_MOBILE_VF_INSIDE_IFRAME_INPUT.SET', () => {
			beforeEach(() => {
				this.beforeEach();
			});
		...
		this.zephyrTest('AutomationAPI', 'PROT-1290', 'FS_MAP_SF_DT_CX_URL_VF_INSIDE_IFRAME_INPUT.SET', () => {
			beforeEach(() => {
				this.beforeEach();
			});
		...
		this.zephyrTest('AutomationAPI', 'PROT-1291', 'FS_MAP_SF_DT_CX_EMAIL_VF_INSIDE_IFRAME_INPUT.SET', () => {
			beforeEach(() => {
				this.beforeEach();
			});
		...
		this.zephyrTest('AutomationAPI', 'PROT-1292', 'FS_MAP_SF_DT_CX_PERCENT_VF_INSIDE_IFRAME_INPUT.SET', () => {
			beforeEach(() => {
				this.beforeEach();
			});
		...
		this.zephyrTest('AutomationAPI', 'PROT-1293', 'FS_MAP_SF_DT_CX_TEXTAREALONG_VF_INSIDE_IFRAME_INPUT.SET', () => {
			beforeEach(() => {
				this.beforeEach();
			});
		...
		this.zephyrTest('AutomationAPI', 'PROT-1294', 'FS_MAP_SF_DT_CX_LOOKUP_VF_INSIDE_IFRAME_INPUT.SET', () => {
			beforeEach(() => {
				this.beforeEach();
			});
		...
		// this.zephyrTest('AutomationAPI', 'PROT-1295', 'FS_MAP_SF_DT_CX_RICHTEXT_VF_INSIDE_IFRAME_INPUT.SET', () => {
		// 	beforeEach(() => {
		// 		this.beforeEach();
		// 	});
		...
		// 	});
		// });
		this.zephyrTest('AutomationAPI', 'PROT-1296', 'FS_MAP_SF_DT_CX_LOOKUP_VF_INSIDE_IFRAME_INPUT.LOOKUP', () => {
			beforeEach(() => {
				this.beforeEach();
			});
			afterAll(async done => {
