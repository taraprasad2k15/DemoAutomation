...
import { QuickLinkData,CaseRL } from './QuickLink.data';
import { join } from 'path';

export class QuickLinkSpec extends BaseSpec {
    public runTest() {
		...
		this.zephyrTest('AutomationAPI', 'PROT-1741', 'FS_MAP_SF_STD_RL_QuickLinks_REFERENCE FIELDS.Custom', () => {
            beforeAll(async done => {
                const debug: boolean = false;
                const tcName = 'QuickLink.testcase';
		...
		this.zephyrTest('AutomationAPI', 'PROT-1742', 'FS_MAP_SF_STD_RL_QuickLinks_TEXT_FIELDS.Custom', () => {
			beforeEach(() => {
				this.beforeEach();
			});
		...
		this.zephyrTest('AutomationAPI', 'PROT-1743', 'FS_MAP_SF_STD_RL_QuickLinks_HEADING HYPERLINKS.Custom', () => {
			beforeEach(() => {
				this.beforeEach();
			});
			this.step('Mapping RL Heading', async () => {
				// await this.findElement(remoteWebdriver, "//span[@title='Cases'] | //div[contains(@class,'previewContainer')]//a[(.)='Cases']");
				// const response: BeginAddToTestCaseResponse = await this.beginAddToTestCase("//span[@title='Cases'] | //div[contains(@class,'previewContainer')]//a[(.)='Cases']");
		...
		this.zephyrTest('AutomationAPI', 'PROT-1744', 'FS_MAP_SF_STD_RL_QuickLinks_COLUMN_ASSERTION.Custom', () => {
			beforeEach(() => {
				this.beforeEach();
			});
			this.step('Mapping Subject Field ', async () => {
		...
		this.zephyrTest('AutomationAPI', 'PROT-1745', 'FS_MAP_SF_STD_RL_QuickLinks_ROW.EditButton.Custom', () => {
			beforeEach(() => {
				this.beforeEach();
			});
	    ...
	    this.zephyrTest('AutomationAPI', 'PROT-1746', 'FS_MAP_SF_STD_RL_QuickLinks_ACTION BUTTONS.Custom', () => {
			beforeEach(() => {
				this.beforeEach();
			});
			afterAll(async done => {
			...
			this.step('Mapping ViewAll button', async () => {
				const QuickLinkWE: WebElement = await this.findElement(remoteWebdriver,"//a/span[contains((.),'Cases')]", 15*1000);
