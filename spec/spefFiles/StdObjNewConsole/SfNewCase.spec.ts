...
export class SfNewCaseConsoleSpec extends BaseSpec {
	public runTest() {
		...
		this.zephyrTest('AutomationAPI', 'PROT-1803', 'SF_MAP_SF_CONSOLE_LX_NEW_OBJ', () => {
			...
			this.step('Mapping Save button field', async () => {
				...
				const element: WebElement = await this.findElement(remoteWebdriver, "//button[@title='Save']/span", 20000);
				const response: BeginAddToTestCaseResponse = await this.beginAddToTestCase("//button[@title='Save']/span");
