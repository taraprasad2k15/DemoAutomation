...
export class FSLBookAppointmentSpec extends BaseSpec {
	public runTest() {
		...
		this.zephyrTest('AutomationAPI', 'PROT-2047', 'FS_MAP_SF_FSL_WO_QA_Book Appointment', () => {
			beforeAll(async done => {
				const debug: boolean = false;
				const tcName = 'FSLBookAppointment.testcase';
				const absPathForTC = join(__dirname, tcName).replace(/\\/g, '/').replace('/target/build/', '/');
				await this.beforeAll(debug, absPathForTC, tcName, RunMode.TestAuthoring, done);
				pageStructureInfo = [FSLBookAppointmentData.PSIBookAppointment];
			}, 200 * 1000);
			beforeEach(() => {
				this.beforeEach();
			});
			...
			this.step('Map getAppointmentBt Field', async () => {
				const response: BeginAddToTestCaseResponse = await this.beginAddToTestCase(FSLBookAppointmentData.getAppointmentBt.xpath, [iframeXpath]);
				this.assertTestBuilderState(pageStructureInfo, FSLBookAppointmentData.getAppointmentBt.fieldDetails, Standard.interaction.click, [], response.testbuilderState);
			});
			this.step('Add & Do getAppointmentBt Field', async () => {
				const response: CompleteAddToTestCaseResponse = await this.completeAddToTestCase(CompleteAction.AddAndDo);
				const testItemId: string = this.miscUtils.findTestItemIdForName(this.testCaseXMLStringTemplate, 'UiWithScreen');
				this.assertExecutionStatus(testItemId, FSLBookAppointmentData.getAppointmentBt.executionStatus, response.runExecutionStatus.currentTestCase);
			});
		});
	};
}; 