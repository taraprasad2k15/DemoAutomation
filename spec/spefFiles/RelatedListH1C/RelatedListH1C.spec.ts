...
import { QuickLinkData,CaseRL } from './RelatedListH1C.data';
import { join } from 'path';

export class RelatedListH1CSpec extends BaseSpec {
    public runTest() {
		...
		this.zephyrTest('AutomationAPI', 'PROT-1762', 'FS_MAP_SF_STD_RL.RP.H1Col', () => {
            beforeAll(async done => {
                const debug: boolean = false;
                const tcName = 'RelatedListH1C.testcase';
			...
			this.step('Mapping RL Heading', async () => {
				await this.findElement(remoteWebdriver, "//h2//span[@title='Cases']");
				const response: BeginAddToTestCaseResponse = await this.beginAddToTestCase("//h2//span[@title='Cases']");
