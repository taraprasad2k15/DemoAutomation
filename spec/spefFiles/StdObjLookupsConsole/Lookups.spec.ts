...
export class SfLookupsConsoleSpec extends BaseSpec {
	public runTest() {
		...
		this.zephyrTest('AutomationAPI', 'PROT-1821', 'SF_MAP_SF_CONSOLE_LX_NEW_OBJ.Lookup', () => {
			beforeAll(async done => {
				const debug: boolean = false;
				const tcName = 'Case New.testcase';
				const absPathForTC = join(__dirname, tcName).replace(/\\/g, '/').replace('/target/build/', '/');
				await this.beforeAll(debug, absPathForTC, tcName, RunMode.TestAuthoring, done);
				pageStructureInfo = [Standard.CurrentTabPsi, Case.caseNewPsi];
				pageStructureInfoLookup = [Standard.CurrentTabPsi, Case.caseLookupPsi];
			}, 300000);
			beforeEach(() => {
				this.beforeEach();
			});
            ...
            this.step('Mapping the Name field', async () => {
                ...
                this.assertTestBuilderState([ Standard.CurrentTabPsi, Lookups.PSIlookup], Lookups.name.FieldDetails, Standard.interaction.click, Standard.applicableInteractions.ReadOnlyLink, response.testbuilderState, rowLocatorLookup);
            });
		...
		this.zephyrTest('AutomationAPI', 'PROT-1820', 'SF_MAP_SF_CONSOLE_LX_IL_OBJ.Lookup', () => {
			beforeAll(async done => {
				const debug: boolean = false;
				const tcName = 'Case View.testcase';
				const absPathForTC = join(__dirname, tcName).replace(/\\/g, '/').replace('/target/build/', '/');
				await this.beforeAll(debug, absPathForTC, tcName, RunMode.TestAuthoring, done);
				pageStructureInfo = [Standard.CurrentTabPsi, Case.caseViewPsi];
				pageStructureInfoLookup = [Standard.CurrentTabPsi, Case.caseLookupPsi];
			}, 200000);
			beforeEach(() => {
				this.beforeEach();
