...
export class QuickLinkConsoleSpec extends BaseSpec {
	public runTest() {
		...
		this.zephyrTest('AutomationAPI', 'PROT-2167', 'FS_MAP_SF_STD_RL_QuickLinks.RP.CPHL', () => {
			beforeAll(async done => {
				const debug: boolean = false;
				const tcName = 'QuickLinkConsole.testcase';
				const absPathForTC = join(__dirname, tcName).replace(/\\/g, '/').replace('/target/build/', '/');
				await this.beforeAll(debug, absPathForTC, tcName, RunMode.TestAuthoring, done);
				pageStructureInfo = [Standard.CurrentTabPsi, QuickLinkData.PSI];
			}, 200 * 1000);
			beforeEach(() => {
				this.beforeEach();
			});
			...
			this.step('Mapping Case Number Field ', async () => {
				...
				const comp: WebElement = await this.findElement(remoteWebdriver, "//article[contains(@class,'headerBottomBorder forceRelatedListCardDesktop')]//div[contains(@class,' forceRelatedListPreview')]", 20*1000);
				await remoteWebdriver.executeScript("arguments[0].scrollIntoView(true)", comp);
				const caseLink: WebElement = await this.findElement(remoteWebdriver, "//a[@force-hoverablelink_hoverablelink]/span[contains(normalize-space(.),'Cases')] | //a[@force-relatedlistquicklink_relatedlistquicklink]/span[contains(normalize-space(.),'Cases')]", 20*1000);
				await remoteWebdriver.executeScript("if(document.createEvent){var evObj = document.createEvent('MouseEvents');evObj.initEvent('mouseover',true, false); arguments[0].dispatchEvent(evObj);} else if(document.createEventObject) { arguments[0].fireEvent('onmouseover');}",caseLink);
				await this.findElement(remoteWebdriver, CaseRL.caseNumber.Xpath, 15 * 1000);
			...
			this.step('Mapping RL Heading', async () => {
				await this.findElement(remoteWebdriver, "//div[contains(@class,'previewContainer')]//a[(.)='Cases']");
				const response: BeginAddToTestCaseResponse = await this.beginAddToTestCase("//div[contains(@class,'previewContainer')]//a[(.)='Cases']");
			...
			this.step('Mapping Edit Button', async () => {
				...
				const editBtn: WebElement = await this.findElement(remoteWebdriver, CaseRL.editButton.Xpath);
			...
			this.step('Mapping Cancel button', async () => {
				...
				this.assertTestBuilderState([Standard.CurrentTabPsi, Case.caseEditPsi], Standard.cancelButton, Standard.interaction.click, Standard.applicableInteractions.Button, response.testbuilderState);
			});
			...
			this.step('Mapping ViewAll button', async () => {
				const QuickLinkWE: WebElement = await this.findElement(remoteWebdriver, "//lst-related-list-quick-links-container//a/span[contains((.),'Cases')] | //a[@force-relatedlistquicklink_relatedlistquicklink]/span[contains(normalize-space(.),'Cases')]", 15 * 1000);
			...
			this.step('Mapping Cancel button', async () => {
				await this.findElement(remoteWebdriver, "//button[@title='Cancel']");
				const response: BeginAddToTestCaseResponse = await this.beginAddToTestCase("//button[@title='Cancel']");
				this.assertTestBuilderState([Standard.CurrentTabPsi, Case.caseNewPsi], Standard.cancelButton, Standard.interaction.click, Standard.applicableInteractions.Button, response.testbuilderState);
			});
			this.step('Add & Do Cancel button', async () => {
				const response: CompleteAddToTestCaseResponse = await this.completeAddToTestCase(CompleteAction.AddAndDo);
				const testItemId: string = this.miscUtils.findTestItemIdForName(this.testCaseXMLStringTemplate, 'UiWithScreen');
