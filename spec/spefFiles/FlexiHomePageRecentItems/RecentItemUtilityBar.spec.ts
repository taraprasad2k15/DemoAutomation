...
export class RecentItemUtillityBarSpec extends BaseSpec {
    public runTest() {
        ...
        this.zephyrTest('AutomationAPI', 'PROT-2150', 'FS_MAP_SF_STD_RECENTITEMS_UtilityBar', () => {
            beforeAll(async done => {
                const debug: boolean = false;
                const tcName = 'RecentItemUtilityBar.testcase';
                const absPathForTC = join(__dirname, tcName).replace(/\\/g, '/').replace('/target/build/', '/');
                await this.beforeAll(debug, absPathForTC, tcName, RunMode.TestAuthoring, done);
                pageStructureInfo = [FlexiHomeRecentItems.scaffold, FlexiHomeRecentItems.PSIUtilityBar];
            ...
            this.step('Perform Click on Account Name Field', async () => {
                ...
                await this.assertTestCaseXML(testItemId, FlexiHomeRecentItems.accountName.testCaseXMLClickUB);
                this.assertExecutionStatus(testItemId, FlexiHomeRecentItems.accountName.executionStatusClickUB, response.runExecutionStatus.currentTestCase);
            });
