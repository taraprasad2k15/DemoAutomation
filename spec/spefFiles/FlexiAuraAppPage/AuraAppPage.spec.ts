...
export class AuraAppPageSpec extends BaseSpec {
    public runTest() {
        const connectionName: string = 'Regmain';
        let remoteWebdriver: WebDriver;
        let pageStructureInfo: any[];
        this.zephyrTest('AutomationAPI', 'PROT-1536', 'FS_MAP_SF_STD_AURA.AP', () => {
            beforeAll(async done => {
                const debug: boolean = false;
                const tcName = 'AuraAppPage.testcase';
                const absPathForTC = join(__dirname, tcName).replace(/\\/g, '/').replace('/target/build/', '/');
                await this.beforeAll(debug, absPathForTC, tcName, RunMode.TestAuthoring, done);
                pageStructureInfo = [AuraAppPageData.Psi.new];
            }, 200 * 1000);
            beforeEach(() => {
                this.beforeEach();
            });
            ...
            this.step('Map Last name field', async () => {
                pageStructureInfo = [AuraAppPageData.Psi.existing];
