...
import { RelatedRecord } from './RelatedRecordHS.data';
import { join } from 'path';

export class RelatedRecordHSSpec extends BaseSpec {
	public runTest() {
		...
		this.zephyrTest('AutomationAPI', 'PROT-1767', 'FS_MAP_SF_STD_RelatedRecord.RP.HSL', () => {
			beforeAll(async done => {
				const debug: boolean = false;
				const tcName = 'RelatedRecordHS.testcase';
			...
			this.step('Mapping removeBt', async () => {
				const dropdownWE = await this.findElement(remoteWebdriver, "//span[contains(text(),'Show available actions')]", 20 * 1000);
				await remoteWebdriver.executeScript("arguments[0].click();", dropdownWE);
				await this.findElement(remoteWebdriver, RelatedRecord.removeBt.xpath);
				const response: BeginAddToTestCaseResponse = await this.beginAddToTestCase(RelatedRecord.removeBt.xpath);
				this.assertTestBuilderState(pageStructureInfo, RelatedRecord.removeBt.fieldDetails, Standard.interaction.click, [], response.testbuilderState);
			});
			this.step('Add & Do removeBt', async () =>{
				await this.completeAddToTestCase(CompleteAction.AddAndDo);
				expect(true).toEqual(true);
			});
			this.step('Mapping searchbox to set Invalid Value', async () =>{
				await this.findElement(remoteWebdriver, RelatedRecord.searchbox.xpath, 20 * 1000);
				const response: BeginAddToTestCaseResponse = await this.beginAddToTestCase(RelatedRecord.searchbox.xpath);
				this.assertTestBuilderState(pageStructureInfo, RelatedRecord.searchbox.fieldDetails, Standard.interaction.set, [], response.testbuilderState);
			});
			this.step('Add & Do searchbox', async () =>{
				await this.modifyAddToTestCase(RelatedRecord.searchbox.modificationParameters);
				await this.completeAddToTestCase(CompleteAction.AddAndDo);
				expect(true).toEqual(true);
			});
			this.step('Mapping searchbox with Error', async () =>{
				await this.findElement(remoteWebdriver, "//ul[contains(@class,'has-error')]");
				const response: BeginAddToTestCaseResponse = await this.beginAddToTestCase(RelatedRecord.searchbox.xpathWithError);
				this.assertTestBuilderState(pageStructureInfo, RelatedRecord.searchbox.fieldDetails, Standard.interaction.set, [], response.testbuilderState);
			});
		});
	};
